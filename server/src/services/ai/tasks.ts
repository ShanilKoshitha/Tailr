...
import {
  ...
  tailorSuggestSchema,
  rewriteOneSchema,
  bulletRelevanceSchema,
  resumeClassifySchema,
} from './schemas.js';
import { modelToPromptText } from '../../docx/parse.js';
import type { ClassifiedPara, JdAnalysis, MatchVerdicts, ResumeModel, Suggestion } from '@tailr/shared';
 ...
 */
export function resumeClassify(
  paragraphs: Array<{ paraId: string; text: string; isBullet: boolean }>,
): Promise<{ paragraphs: ClassifiedPara[] }> {
...
PARAGRAPHS:
${paragraphs
  .map((p) => `[${p.paraId}]${p.isBullet ? ' (isBullet)' : ''} ${p.text || '(empty)'}`)
  .join('\n')
  .slice(0, 24000)}`;
...
export function bulletRelevance(
  model: ResumeModel,
  jd: JdAnalysis,
): Promise<{ bullets: Array<{ paraId: string; relevance: number; reason?: string }> }> {
...
JOB: ${jd.titleEssence}
KEY ITEMS: ${[...jd.qualifications, ...jd.responsibilities]
    .map((i) => i.text)
    .join(' | ')
    .slice(0, 4000)}
...
export function tailorSuggest(opts: {
  ...
  const summaryIds = [
    ...(opts.model.sections['summary'] ?? []),
    ...(opts.model.sections['header'] ?? []),
  ];
  const summaryPara = model.paragraphs.find(
    (p) => summaryIds.includes(p.paraId) && p.kind === 'body' && p.text.length > 120,
  );
...
${gaps
  .map(
    (g) =>
      `${g.itemId} (${g.verdict}): ${itemText(g.itemId)}${g.suggestionHint ? ` — hint: ${g.suggestionHint}` : ''}`,
  )
  .join('\n')
  .slice(0, 8000)}
