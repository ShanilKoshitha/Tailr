...
export function heuristicsFailed(model: ResumeModel): boolean {
  const meaningful = Object.entries(model.sections).filter(
    ([name, ids]) => name !== 'header' && ids.length > 0,
  ).length;
  return meaningful < 2 || model.entries.length === 0;
}

/** Returns a re-labeled model, or the original if AI is unavailable/fails. */
export async function aiClassifyModel(
  model: ResumeModel,
): Promise<{ model: ResumeModel; usedAi: boolean }> {
  if (!heuristicsFailed(model)) return { model, usedAi: false };
  let labels;
  try {
    labels = await resumeClassify(
      model.paragraphs.map((p) => ({ paraId: p.paraId, text: p.text, isBullet: p.isBullet })),
    );
  } catch {
  ...
  const entryMap = new Map<
    string,
    { entryId: string; headerId: string; title: string; company: string; bulletIds: string[] }
  >();

  for (const p of model.paragraphs) {
    const l = byId.get(p.paraId);
    const kind = (
      p.isBullet ? 'bullet' : (l?.kind ?? p.kind)
    ) as ResumeModel['paragraphs'][number]['kind'];
