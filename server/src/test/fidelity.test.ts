...
const inputs = process.argv.slice(2).length ? process.argv.slice(2) : [path.join(ROOT, 'shanil-hewage.docx')];
...
async function toPngs(pdf: string, prefix: string): Promise<string[]> {
  ...
  return fs.readdirSync(dir).filter((f) => f.startsWith(base) && f.endsWith('.png')).sort().map((f) => path.join(dir, f));
}

let failed = false;
for (const input of inputs) {
  if (!fs.existsSync(input)) { console.log(`skip (missing): ${input}`); continue; }
  ...
  try {
    ...
    const [aPngs, bPngs] = [await toPngs(aPdf, path.join(tmp, 'pa')), await toPngs(bPdf, path.join(tmp, 'pb'))];
    ...
    for (let i = 0; i < aPngs.length; i++) {
      ...
      console.log(`  ${path.basename(input)} page ${i + 1}/${aPngs.length}: ${pct.toFixed(3)}% match (${diff} px differ)`);
    }
    const ok = worst >= THRESHOLD;
    console.log(`${ok ? '✓' : '✗'} ${path.basename(input)}: worst page ${worst.toFixed(3)}% (threshold ${THRESHOLD}%)`);
    if (!ok) failed = true;
  } finally {
