...
export default function ResumesView() {
  ...
  const load = useCallback(() => api.get<Resume[]>('/api/resumes').then(setResumes).catch((e) => toast(e.message, 'error')), []);
  useEffect(() => { load(); }, [load]);
  ...
  async function handleFiles(files: FileList | null) {
    const file = files?.[0];
    if (!file) return;
    if (/\.pdf$/i.test(file.name)) { setPdfRejected(true); return; }
    setUploading(true);
    try {
  ...
  return (
    <div className="mx-auto h-full max-w-3xl overflow-y-auto thin-scroll px-6 py-8">
      ...
      <p className="mt-1 text-sm text-slate-500">
        Upload your own DOCX. Tailr never re-templates it — every AI edit only touches the text of targeted lines.
      </p>
      ...
      >
        {uploading ? <Spinner className="h-8 w-8 text-brand-500" /> : (
          <>
        ...
        <input ref={fileRef} type="file" accept=".docx" hidden onChange={(e) => handleFiles(e.target.files)} />
      </div>

      <div className="mt-6 space-y-2">
        {resumes === null ? <Spinner className="h-5 w-5 text-brand-500" /> :
          resumes.length === 0 ? (
            <EmptyState icon="🪡" title="No resumes yet">Your uploaded resume becomes the base for every tailored version.</EmptyState>
      ...
      <Modal open={pdfRejected} onClose={() => setPdfRejected(false)} title="DOCX needed for tailoring">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Export your resume as <b>.docx</b> (Word: File → Save As · Google Docs: File → Download → Microsoft Word) and re-upload.
        </p>
        <div className="mt-4 flex justify-end"><Button onClick={() => setPdfRejected(false)}>Got it</Button></div>
      </Modal>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white">
      <div className="flex items-center gap-3 p-4">
        ...
        <Button variant="ghost" onClick={() => setOpen((v) => !v)}>{open ? 'Hide preview' : 'Preview'}</Button>
        <Button variant="subtle" onClick={remove}>Delete</Button>
      </div>
      {open && (
        <div className="border-t border-slate-100 p-3">
          <iframe title="preview" src={`/api/resumes/${resume.id}/preview.pdf`} className="h-[560px] w-full rounded-lg border border-slate-200" />
          <p className="mt-1 text-center text-xs text-slate-400">Preview requires LibreOffice — see Settings if blank.</p>
        </div>
      )}
    </div>
  );
