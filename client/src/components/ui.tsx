...
export function Button({
  ...
  return (
    <button
      {...props}
      className={`inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors disabled:cursor-not-allowed ${styles} ${className}`}
    >
      {children}
    </button>
  );
...
export function Chip({
  ...
  children: React.ReactNode;
  tone?: 'slate' | 'green' | 'amber' | 'red' | 'brand' | 'violet';
  title?: string;
  onClick?: () => void;
}) {
  const tones = {
  ...
  return (
    <p className="text-sm leading-relaxed">
      {parts.map((p, k) => {
        ...
        if (p.type === 'del')
          return (
            <span key={k} className="bg-rose-50 text-rose-500 line-through decoration-rose-300">
              {text}
            </span>
          );
        return (
          <span key={k} className="bg-emerald-50 font-medium text-emerald-700">
            {text}
          </span>
        );
      })}
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-[100] flex flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pop-in pointer-events-auto rounded-xl px-4 py-2.5 text-sm font-medium shadow-lg ${
            t.tone === 'error'
